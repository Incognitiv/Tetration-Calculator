...
// Constant messages
const WELCOME:                  &str = "Welcome to the Tetrator Calculator!";                                   // Welcome message
const EXIT:                     &str = "Exit";                                                                  // Option to exit the program
const BYE:                      &str = "Goodbye!";                                                              // Farewell message

const OPTION_SELECT:            &str = "Select an option:";                                                     // Menu prompt
const ENTER_BASE:               &str = "Please enter the base (a non-negative integer):";                       // Prompt for base input
const ENTER_HEIGHT:             &str = "Please enter the height (a non-negative integer):";                     // Prompt for height input
const START_COUNTING:           &str = "Perform Tetration";                                                     // Option to start tetration calculation

const INVALID_BASE_INPUT:       &str = "Invalid base input:";                                                   // Error message for invalid base input
const INVALID_HEIGHT_INPUT:     &str = "Invalid height input:";                                                 // Error message for invalid height input
const INVALID_SELECTION:        &str = "Invalid selection. Please choose a valid option:";                      // Error message for invalid menu selection
const INVALID_BASE_MESSAGE:     &str = "Invalid input for the base. Please enter a positive integer.";          // Error message for invalid base input format
const INVALID_HEIGHT_MESSAGE:   &str = "Invalid input for the height. Please enter a non-negative integer.";    // Error message for invalid height input format
const INVALID_BASE_AND_HEIGHT:  &str = "Invalid base and height inputs:";                                       // Error message for invalid base and height inputs
const INVALID_INPUT_UNREADABLE: &str = "Failed to read input from the user.";                                   // Error message for unreadable input
const ERROR_OCCURRED:           &str = "Error occurred during tetration calculation.";                          // Error message for a calculation error

// Other constant values
const ZERO:                     u128 = 0;                                                                       // Constant value for zero
const ONE:                      u128 = 1;                                                                       // Constant value for one

// Define a struct called TetratorCalculator, which represents a calculator for tetrations
struct TetratorCalculator {
...
impl TetratorCalculator {
    ...
    pub fn new() -> Self {
         ...
         TetratorCalculator {
            ...
            height: ZERO,  // Initializes the "height" field of the TetratorCalculator with a value represented by the constant ZERO
        }
    }
...
// The "calculator" parameter is a mutable reference to a TetratorCalculator
...
fn validate_input<F>(calculator: &mut TetratorCalculator, prompt: &str, error_message: &str, setter: F, ) -> Result<(), String> where F: Fn(&mut TetratorCalculator, u128) {
...
fn display_tetration_result(calculator: &TetratorCalculator, result: &BigUint, elapsed_time: &std::time::Duration) {
...
fn calculate_tetration(calculator: &mut TetratorCalculator) {

    // Attempt to validate and set both the base and height values. The result is a tuple
    match (validate_input(calculator, ENTER_BASE, INVALID_BASE_MESSAGE, TetratorCalculator::set_base), validate_input(calculator, ENTER_HEIGHT, INVALID_HEIGHT_MESSAGE, TetratorCalculator::set_height)) {
...
fn main() {
    ...
    // Create a new instance of the TetratorCalculator to perform tetration calculations
    let mut calculator = TetratorCalculator::new();
